...
import net.TammamDarwish.springboot_blog_webApp.util.ROLE;
...
public class PostController {
	...
	// create Handler Method to handle Model and view GET the request and return
	// Model and view
	...
	public String createPost(@Valid @ModelAttribute("postDto") PostDto postDto, BindingResult result, Model model) {
		if (result.hasErrors()) {
			model.addAttribute("postDto", postDto);
			return "admin/create_post";
		}
	...
	/*
	 * we should make the validation in the PostDto object because we use it as
	 * object in the create_post html file
	 * 
	...
	/*
	 * 1-add the dependency in the maven (pom.xml) 2-add NotEmpty annotation above
	 * the the fields in the POSTDTO object 3-Enable annotation in the handler
	 * method @Valid @ModelAttribute("postDto") PostDto postDto 4-use Binding result
	 * object and return to the same page if there is any error "admin/create_post"
	 */
